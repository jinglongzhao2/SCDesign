...
for(K.temp in changing.K)
{
  ...
  if(K.temp == 1)
  {
    ...
    my.png.name = as.character(paste0("output/Fitted_Constrained_K=", K.temp, "_", format(Sys.time(), "%Y%m%d"), ".png"))
    png(filename = my.png.name, width = 1536, height = 768)
    par(mar=c(9.5,4,0.5,0.5))
    plot(1:T.total, rep(0, T.total), ylim = c(plot.l.lim, plot.u.lim), xlab = "", xaxt = 'n', ylab = "", type = 'l', lty = 2, lwd = 3, main = "", cex.lab = 2, cex.axis = 2)
    axis(1, at = (1:T.total)[seq(1, T.total, by = 4)], labels = unique.date.names[seq(1, T.total, by = 4)], las = 2, cex.axis = 1.5)
    ...
    lines(1:T.naught, c(C.fitted.N.before.Cardinality.matrix[K.temp,], C.N.after.Cardinality.matrix[K.temp,])[1:T.naught], type = 'l', lty = 2, lwd = 3)
    lines(1:T.naught, c(T.fitted.N.before.Cardinality.matrix[K.temp,], T.I.after.Cardinality.matrix[K.temp,])[1:T.naught], type = 'l', lty = 1, lwd = 3)
    ...
    my.png.name = as.character(paste0("output/Residuals_Constrained_K=1_", format(Sys.time(), "%Y%m%d"), ".png"))
    png(filename = my.png.name, width = 1536, height = 768)
    par(mar=c(9.5,4,0.5,0.5))
    plot(1:T.total, rep(plot.u.lim, T.total) * 2, ylim = c(plot.l.lim, plot.u.lim), xlab = "", xaxt = 'n', ylab = "", type = 'l', lwd = 3, main = "", cex = 3, cex.lab = 2, cex.axis = 2)
    lines(1:T.naught, c(T.fitted.N.before.Cardinality.matrix[K.temp,1:T.naught] - C.fitted.N.before.Cardinality.matrix[K.temp,1:T.naught]), type = 'l', lwd = 3, main = "", cex = 3, cex.lab = 2, cex.axis = 2)
